...
public class GwtcMonitorService implements ServletContextListener {
  ...
  protected static class CompileTask implements Runnable {
    ...
    public void run() {
      ...
      while (!compileRequest.isFinished()) {
        ...
        synchronized(compileWatchers) {
          watchers = compileWatchers.remove(compileRequest.getId());
        }
        if (watchers == null) {
          trySleep(500, 0);
          continue;
  ...
  protected static class MessengerTask implements Runnable {
    ...
    private void flushMessages(PrintWriter writer, ServletResponse response) {
  ...
  protected String getGwtVersion() {
    try {
      return GwtcSettings.getGwtVersion();
    } catch (Throwable e) {
      return "2.7.1";
    }
  }
