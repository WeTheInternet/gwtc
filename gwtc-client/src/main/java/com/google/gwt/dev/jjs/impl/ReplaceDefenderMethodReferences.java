      ...
      if (x.getTarget().getName().startsWith(lambdaName)) {
        ...
        for (JInterfaceType iface : lambdaType.getImplements()) {
          ...
          if (sam != null) {
            ...
            JStatement expr = block.getStatements().get(0);
            if (expr instanceof JReturnStatement) {
              JExpression lambdaExpr = ((JReturnStatement)expr).getExpr();
              new LambdaRewriteVisitor(lambdaType).accept(lambdaExpr);
            } else {
              JExpressionStatement lambdaExpr = (JExpressionStatement)expr;
              new LambdaRewriteVisitor(lambdaType).accept(lambdaExpr);
            }
            return true;
          }
